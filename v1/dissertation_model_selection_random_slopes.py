#!/usr/bin/env python3
"""Backward selection with a profession-group mixed model with random slopes.

This variant mirrors dissertation_model_selection.py but uses a model whose
random-effects structure uses profession as the grouping factor and includes
random slopes for semantic_role, syntactic_role, valence, and dominance.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run backward selection from a profession-slopes mixed model."
    )
...
def sanitized_run_dir(input_path: Path, output_root: Path) -> Path:
    ...
    return output_root / "model_selection_random_slopes" / f"{safe_stem}__{digest}"
...
def extract_random_effect_variances(result: Any) -> dict[str, float]:
    cov_re = pd.DataFrame(np.asarray(result.cov_re), index=result.cov_re.index, columns=result.cov_re.columns)
    variances: dict[str, float] = {}
    for term_name in ["semantic_role", "syntactic_role", "valence", "dominance"]:
        matches = [label for label in cov_re.index if str(label).startswith(f"C({term_name})")]
        if matches:
            key = matches[0]
            variances[f"random_effect_variance_{term_name}"] = float(cov_re.loc[key, key])
    return variances
...
def fit_mixed_model_terms(
    ...
    *,
    ...
    spec = ModelSpec(name=name, fixed_predictors=[], group_col=group_col, random_predictors=random_predictors, pairwise_interactions=False)
    ...
    for method in ["lbfgs", "bfgs", "cg", "powell", "nm"]:
        try:
            with warnings.catch_warnings(record=True) as caught_warnings:
                ...
                model = smf.mixedlm(formula=formula, data=df, groups=df[spec.group_col], re_formula=re_formula)
            ...
            pd.DataFrame.from_dict(result.random_effects, orient="index").to_csv(model_dir / "group_random_effects.csv")
...
def build_report_reuse_payload(
    ...
    *,
    ...
    return {
        ...
        "random_effect_variance_semantic_role": metrics.get(
            "random_effect_variance_semantic_role"
        ),
        "random_effect_variance_syntactic_role": metrics.get(
            "random_effect_variance_syntactic_role"
        ),
        "random_effect_variance_valence": metrics.get("random_effect_variance_valence"),
        "random_effect_variance_dominance": metrics.get(
...
def can_drop_term(current_terms: list[str], term_to_drop: str) -> bool:
    if ":" not in term_to_drop:
        return not any(term_to_drop in interaction.split(":") for interaction in current_terms if ":" in interaction)
    return True
...
def select_backward_model(
    ...
    while True:
        ...
        if not candidates:
            break
        best_p, best_term, _, best_fit = max(candidates, key=lambda item: item[0])
