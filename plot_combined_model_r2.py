#!/usr/bin/env python3
"""Plot grouped R² values from the completed profession-model reports.

The plot reads the precomputed ``model_metrics.csv`` files written by
``run_knowledge_perception_profession_report.py`` and
``run_lasso_profession_embeddings_report.py``.  For every target language
model, it places the Profession-Embedding, World-Knowledge, and
Human-Perception R² bars next to one another.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--profession-metrics",
        type=Path,
        default=Path("profession_embedding_model_selection_report/model_metrics.csv"),
        help="Precomputed metrics from the profession-embedding report.",
    )
...
def build_combined_data(knowledge_path: Path, profession_path: Path) -> pd.DataFrame:
    knowledge = pd.read_csv(knowledge_path)
    profession = pd.read_csv(profession_path)
    require_columns(knowledge, {"target_model", "model_key", "R2m"}, knowledge_path)
    require_columns(profession, {"target_model", "R2m"}, profession_path)
    ...
    profession = profession[["target_model", "R2m"]].copy()
    profession["target_model"] = profession["target_model"].map(clean_model_name)
    profession = profession.drop_duplicates("target_model").set_index("target_model").rename(
        columns={"R2m": "profession_embedding"}
    )
...
def main() -> int:
    args = parse_args()
    for path in [args.knowledge_metrics, args.profession_metrics]:
        if not path.exists():
            raise FileNotFoundError(f"Metrics file not found: {path}")
    combined = build_combined_data(args.knowledge_metrics, args.profession_metrics)
