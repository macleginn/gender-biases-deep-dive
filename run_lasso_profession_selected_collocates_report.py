...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--collocate-list", type=Path, action="append", default=None,
                        help="Text file of allowed collocates (may be supplied more than once).")
    parser.add_argument("--data-dir", type=Path, default=Path("profession_selected_collocates_data"))
    parser.add_argument("--report-dir", type=Path, default=Path("profession_selected_collocates_report"))
...
def load_model_display_config() -> tuple[list[str], dict[str, str]]:
    ...
    if len(model_ids) != len(model_labels):
        raise ValueError("full_model_list.txt and model_names.txt must contain the same number of non-empty lines.")
...
def standardize_predictors(df: pd.DataFrame, columns: list[str], *, source: Path) -> dict[str, dict[str, float]]:
    ...
    for column in columns:
        ...
        if not np.isfinite(std) or std == 0:
            raise ValueError(f"{source} has no usable variation in {column} for standardization.")
...
def compute_profession_embeddings(path: Path, k: int, method: str = "tfidf",
                                  allowed_collocates: set[str] | None = None) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if allowed_collocates is not None:
        selected_columns = [column for column in collocate_columns if column in allowed_collocates]
        if not selected_columns:
            raise ValueError(f"None of the collocates in {path} are present in the supplied vocabularies")
        collocates = collocates[[profession_column, *selected_columns]]
        collocate_columns = selected_columns
    counts = collocates.loc[:, collocate_columns].apply(pd.to_numeric, errors="coerce").fillna(0.0)
    if (counts < 0).any().any():
...
def decompose_r_squared(df: pd.DataFrame, selected: dict[str, Any], terms: list[str],
    ...
    for predictor in terms:
        ...
        formula = "log_he_she_odds ~ " + (" + ".join(reduced_terms) if reduced_terms else "1")
    ...
    decomposition = decomposition.sort_values("unique_r2", ascending=False).reset_index(drop=True)
    decomposition.to_csv(run_dir / "r2_decomposition.csv", index=False)
    return decomposition


def select_model(df: pd.DataFrame, run_dir: Path, embedding_columns: list[str], maxiter: int,
    ...
    selected = fit_model(df, "selected_profession_embedding", selected_terms,
...
def run_one(input_csv: Path, embedding: pd.DataFrame, embedding_meta: dict[str, Any], data_dir: Path,
    ...
    selected, terms, r2_decomposition = select_model(df, run_dir, embedding_columns, maxiter, interactions)
    ...
    (run_dir / "run_summary.json").write_text(json.dumps({"input_csv": str(input_csv.resolve()),
        "embedding": embedding_meta, "selected_terms": terms,
        "selected_predictor_count": int(len(r2_decomposition)),
        "selected_model": selected["name"], "baseline_model": baseline["name"],
        "preprocessing": preprocessing}, indent=2), encoding="utf-8")
    return run_dir
...
def main() -> int:
    if IMPORT_ERROR is not None:
        ...
        return 1
    args = parse_args()
    collocate_list_paths = args.collocate_list or [Path("collocates_gendered.txt"), Path("collocates_names.txt")]
    ...
    if args.reuse_existing:
        ...
        run_dirs = []
        meta = {"k": args.k, "source": str(args.profession_collocates.resolve()),
                "allowed_collocate_files": [str(path.resolve()) for path in collocate_list_paths],
                "allowed_collocate_count": len(allowed_collocates), "methods": EMBEDDING_METHODS}
        for method in EMBEDDING_METHODS:
            embedding, method_meta = compute_profession_embeddings(
                args.profession_collocates, args.k, method, allowed_collocates
            )
            method_meta["allowed_collocate_files"] = [str(path.resolve()) for path in collocate_list_paths]
